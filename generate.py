...
##    bundle = torchaudio.pipelines.WAV2VEC2_ASR_BASE_960H ## for ENGLISCH
## deutsches model gibt es. byw. gewichtetes multilingual: VOXPOPULI_ASR_BASE_10K_DE
...
def main(audio_file): 
  ...
  path = os.path.join(os.getcwd(), str(audio_file))
  ...
  for i,segment in enumerate(segments):
    text = segment["text"]
    audioSegment = AudioSegment.from_wav(str(audio_file))[segment["start"]*1000:segment["end"]*1000]
    ...
    print(segment["start"])
    subs = force_align(str(i)+'.wav', transcript.lower(), start_index, segment["start"])
